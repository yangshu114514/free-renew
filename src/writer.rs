...
//!
//! 生成规范来自用户长期过审实践（2026-09-12 定稿）：克制、务实、略带极客幽默的
//! 第一人称实测笔记——此前两版都不行：模板吹捧版被平台判 AI 文删除；矫枉过正的
//! 吐槽版又滑向抱怨。校准 = 负面具体且克制、正面不绝对化、按模块组织、1200-1800 字。
//! 规范要点硬编码进 validate()（红线：绝对化表述/竞品名/虚构负面/生产级暗示），
//! 可通过 config.toml [ai] 覆盖/追加。
